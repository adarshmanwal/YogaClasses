...
export default function ShopDetails() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await httpClient.put(`/shops/update/${shopData.id}`, shopData);
      if (response.status === 200) {
        shopCtx.updateShop(response.data.data);
        setEditShop(false);
  ...
  return (
    <>
        ...
        <Modal open={editShop} onClose={() => setEditShop(false)}>
          <form onSubmit={handleSubmit} className="p-4 bg-white rounded-md shadow-lg">
            ...
            {[
              ...
              { name: "closingHours", type: "text", placeholder: "Closing Hours" },
            ...
            <input type="file" accept="image/*" onChange={handleFileChange} className="border p-2 w-full mb-4" />
            <div className="flex justify-end gap-2">
              <button type="button" className="bg-gray-200 px-4 py-2 rounded-md" onClick={() => setEditShop(false)}>
                Cancel
              </button>
      ...
      <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-8">
        <img src={shopData.image || "https://via.placeholder.com/600x300"} alt={shopData.name} className="w-full h-64 object-cover rounded-lg" />
        <div className="mt-4">
          <h1 className="text-2xl font-bold">{shopData.name}</h1>
          <p className="text-gray-600">{shopData.description}</p>
        </div>
        <div className="mt-4 flex justify-end space-x-4">
          <button onClick={() => setEditShop(true)} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
            Edit
          </button>
        ...
        <div className="mt-4 border-t pt-4 text-gray-700">
          ...
          <p><strong>📞 Contact:</strong> {shopData.phoneNumber} | {shopData.email}</p>
          <p><strong>⏰ Open Hours:</strong> {shopData.openingHours} - {shopData.closingHours}</p>
        </div>
      </div>
    </>
  );
